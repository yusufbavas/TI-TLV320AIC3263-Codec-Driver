			    ...
			    struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_update_bits(codec, AIC3262_HP_CTL,
				AIC3262_HP_STAGE_MASK ,
				AIC3262_HP_STAGE_25 << AIC3262_HP_STAGE_SHIFT);
		snd_soc_component_read(codec, AIC3262_HP_AMP_CNTL_R1,&hpl_hpr);
		if((hpl_hpr & 0x3) == 0x3) {
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		snd_soc_component_read(codec, AIC3262_POWER_CONF,&hpl_hpr);
	...
	void *context)
{
	...
	if (fw != NULL) {
		dev_err(codec->dev, "Firmware binary load\n");
				   ...
				   struct snd_soc_jack *jack, int report)
{
	...
	/* Sync status */
	snd_soc_component_read(codec, AIC3262_DAC_FLAG,&status);
			...
			struct snd_soc_dai *dai)
{
	...
	snd_soc_component_read(codec, AIC3262_ASI1_BUS_FMT,&val);
	snd_soc_component_read(codec, AIC3262_ASI1_CHNL_SETUP,&val);
				  ...
				  enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		...
		dev_err(codec->dev, "set_bias_on\n");
		break;

		/* partial On */
	case SND_SOC_BIAS_PREPARE:
		dev_err(codec->dev, "set_bias_prepare\n");
		break;
	...
	case SND_SOC_BIAS_STANDBY:
		 ...
		 */
		dev_err(codec->dev, "set_bias_stby\n");
	...
	case SND_SOC_BIAS_OFF:
		dev_err(codec->dev, "set_bias_off\n");
		/* force all power off */
...
static void aic3262_codec_remove(struct snd_soc_component *codec)
{
	...
	case TLV320AIC3262:
		...
		break;
	default:
		dev_err(codec->dev, "Codec is not TLV320AIC3262\n");
	}
